...
export function TraitementDemande(){
    ...
    // === Vérification d'anomalie EN TEMPS RÉEL (debounce 1s après la dernière frappe) ===
    const debounceAnomalieRef = useRef({}); // { [rowKey]: timeoutId }
    const dernierPrixVerifieRef = useRef({}); // { [rowKey]: dernier prix déjà envoyé en vérification }

    useEffect(() => {
        lignes.forEach(({ rowKey, lot }) => {
            const prix = resultatsParArticle[rowKey]?.puAriary;

            // Annule toute vérification déjà programmée pour cette ligne (l'utilisateur retape encore)
            if (debounceAnomalieRef.current[rowKey]) {
                clearTimeout(debounceAnomalieRef.current[rowKey]);
            }

            // Rien à vérifier tant que le prix n'est pas calculable
            if (!prix || prix <= 0) return;

            // Évite de relancer une vérification identique (ex: re-render sans changement réel de prix)
            if (dernierPrixVerifieRef.current[rowKey] === prix) return;

            debounceAnomalieRef.current[rowKey] = setTimeout(() => {
                dernierPrixVerifieRef.current[rowKey] = prix;
                verifierAnomalie(rowKey, lot.designation, lot.codeLot, prix);
            }, 1000); // 1 seconde de silence avant de vérifier automatiquement
        });

        // Nettoyage : annule les timers en attente si le composant est démonté
        return () => {
            Object.values(debounceAnomalieRef.current).forEach(clearTimeout);
        };
    }, [resultatsParArticle, lignes]);
                                                ...
                                                <tr key={rowKey}>
                                                    <td>
                                                        ...
                                                        <button 
                                                            ...
                                                            title={!r.puAriary || r.puAriary <= 0 ? "Renseignez d'abord la quantité et le prix unitaire" : "Revérifier immédiatement (vérification automatique après 1s d'inactivité)"}
                                                        >
                                                             {loadingAnomalie[rowKey] ? "Vérification..." : "Revérifier"}
                                                        </button>
                                                    </td>
                                                    <td>
                                                        <input
