...
namespace GestPR.Service
{
    public class DemandeService : IDemandeService
    {
        ...
        public DemandeService(IDemandeRepository repo, IEmailService emailService, IConfiguration config, GestPR.Data.AppDbContext context, IHttpContextAccessor httpContextAccessor)
        {
            _repo = repo;
        ...
        public async Task<bool> SoumettreDemandeAsync(int id, IFormFile pdfFile, string articlesJson, string? commentaire, string? typeDossier, string? immo)
        {
            var demande = await _repo.GetByIdAsync(id);
            ...
            try
            {
                ...
                if (emailsValidateurs.Count > 0)
                {
                    ...
                    foreach (var art in demande.Articles)
                    {
                        ...
                        // Si 'immo' n'a pas été passé en paramètre global, tentez de le récupérer depuis les DTOs d'articles
                        string immoArticle = !string.IsNullOrWhiteSpace(immo)
                            ? immo
                            : (!string.IsNullOrWhiteSpace(artSaisi?.Immo) ? artSaisi.Immo : "Non défini");
